...
def eval_scib_metrics(
    ...
    if "neighbors" in adata.uns:
        ...
        log.info(
            "neighbors in adata.uns removed, new neighbors calculated: "
    ...
    if len(adata.obs[batch_key].unique()) > 1:
        results_dict["ASW_batch"] = scib.metrics.silhouette(
            adata, batch_key, embedding_key, "euclidean"
        )
        ...
        results_dict["PCR_batch"] = scib.metrics.pcr_comparison(
            adata,
            adata,
            covariate=batch_key,
            embed=embedding_key,
...
def create_attention_mask_modified(
    unknown_genes: torch.Tensor, cell_embedding_position: int = 0
) -> torch.Tensor:
    """
    Create an attention mask from a vector of positions of unknown genes.
    ...
    if unknown_genes.device.type == "cpu":
        log.warning(
            "create_attention_mask_modified() is not optimized for CPU, "
    ...
    for i in range(len(unknown_genes)):
        attn_mask[i][
            unknown_genes[i].unsqueeze(-1) & known_genes[i].unsqueeze(0)
        ] = True
...
def mask_data(
    ...
    gene_mask = torch.logical_and(
        tokenized_data["values"].ne(pad_value), masked_values.eq(mask_value)
    )
...
def calculate_losses(
    ...
    methods_ = [
        method for method in methods if method not in implemented_methods
    ]
    ...
    if skip_cell:
        ...
        masked_position = torch.concat(
            (masked_position[:, :cp], masked_position[:, cp + 1 :]), dim=1
        )
        non_padded_position = torch.concat(
            (non_padded_position[:, :cp], non_padded_position[:, cp + 1 :]),
            dim=1,
        )
    ...
    if "mse" in methods:
        ...
        loss = F.mse_loss(
            torch.masked_select(output, masked_position).float(),
        ...
        loss_all = F.mse_loss(
            torch.masked_select(output, non_padded_position).float(),
    ...
    if "mre" in methods:
        loss = torch.abs(
            torch.masked_select(output, masked_position)
        ...
        loss_all = torch.abs(
            (
                torch.masked_select(output, non_padded_position)
...
def permute_values(
    mat: torch.Tensor, pad_value: int = -2, cell_embedding: bool = True
) -> tuple:
    """
    ...
    if cell_embedding:  # TODO: test not permuting cell_embedding!
        ...
        indx_perm = torch.cat(
            (
                torch.zeros(indx_perm.shape[0], 1).to(indx_perm.device),
                indx_perm + 1,
            ),
            dim=1,
        )
...
def rearrange(mat: torch.Tensor, indx: torch.Tensor) -> torch.Tensor:
    """
...
def reverse_permute(mat: torch.Tensor, indx: torch.Tensor) -> torch.Tensor:
    """
