...
def save_tensor_as_csv(
    tensor: torch.Tensor, output_dir: str, filename: str
) -> None:
    """
...
def calculate_correlation(in_ranks_mat, out_ranks_mat, mean_ranks_mat):
    ...
    if not (in_ranks_mat.shape == out_ranks_mat.shape == mean_ranks_mat.shape):
        msg = (
            f"Input ranks matrix shape: {in_ranks_mat.shape}, "
...
class GeneExprPredEval:
    def __init__(
        self,
        model_instance: Union[scGPT_instance, Geneformer_instance],
        ...
        if not isinstance(
            model_instance, (scGPT_instance, Geneformer_instance)
        ):
    ...
    def _evaluate_Geneformer(
        ...
        if n_cells is None:
            ...
            msg = (
                f"n_cells {n_cells} is larger than the number of cells in the "
        ...
        if n_cells < n_all_cells:
            rand_cells = np.random.choice(
                range(n_all_cells), n_cells, replace=False
            )
            input_rankings = [
                self.eval_instance.input_rankings[i] for i in rand_cells
            ]
            output_rankings = [
                self.eval_instance.output_rankings[i] for i in rand_cells
            ]

            output_rankings = [
                output_rankings[i][: len(input_rankings[i])]
            ...
            output_rankings = [
                self.eval_instance.output_rankings[i][: len(input_rankings[i])]
                for i in range(len(input_rankings))
            ]
        ...
        if (
            "adata_order"
            not in self.eval_instance.tokenized_dataset.features.keys()
        ):
            msg = "adata_order is not in the dataset; generating numbers for sample_id"
            log.warning(msg)

            sample_ids = [
                [i] * input_rankings[i].shape[0]
                for i in range(len(input_rankings))
            ]
        else:
            sample_ids = [
                self.eval_instance.tokenized_dataset.select([i])["adata_order"]
                for i in rand_cells
            ]
            sample_ids = [
        ...
        positions = [
            np.arange(input_rankings[i].shape[0])
            for i in range(len(input_rankings))
        ]
        positions = [
            np.max(positions[i]) - positions[i]
            for i in range(len(input_rankings))
        ]
        positions = [
            positions[i] / np.max(positions[i])
            for i in range(len(input_rankings))
        ]
        ...
        output_df = (
            output_df.groupby(["token", "sample_id"]).mean().reset_index()
        )

        # join the two dataframes
        df = pd.merge(
            input_df, output_df, on=["token", "sample_id"], how="outer"
        )
        ...
        mean_rank_df = (
            df[df["input_rank"] > 0]
            .groupby(["token"])
            .apply(lambda x: x.input_rank.mean())
            .reset_index()
        )
        # rename the column
        ...
        cors = df.groupby("sample_id").apply(
            lambda x: stats.pearsonr(x["input_rank"], x["output_rank"])[0]
        )
        mean_cors = df.groupby("sample_id").apply(
            lambda x: stats.pearsonr(x["input_rank"], x["mean_rank"])[0]
        )

        cors_df = pd.concat([cors, mean_cors], axis=1)
        cors_df = cors_df.rename(
            columns={0: "correlation", 1: "mean_correlation"}
        )
        ...
        if save_rankings:
            df.to_csv(
                os.path.join(self.output_dir, "rankings.csv.gz"),
                index=False,
                compression="gzip",
            )
        ...
        if self._wandb:
            self._wandb.log(
                {
                    "gene_embeddings_metrics": self._wandb.Table(
        ...
        if return_all:
            cors_df.to_csv(
                os.path.join(self.output_dir, "correlations.csv.gz"),
                index=False,
                compression="gzip",
            )
    ...
    def _evaluate_scGPT(
        ...
        if include_zero_genes:
            masked_values_ = torch.full_like(
                input_values, True, dtype=torch.bool
            )
        else:
            masked_values_ = input_values > 0

        non_padded_values = input_values.ne(
            self.eval_instance.model_config["pad_value"]
        )
        ...
        for emb_key in self.embedding_key:
            ...
            metrics = utils.calculate_losses(
                embedding,
                input_values,
                masked_values_,
                non_padded_values,
            ...
            metrics_df = pd.concat(
                [metrics_df, pd.DataFrame(metrics, index=[emb_key])]
            )
        ...
        metrics = {key: value.item() for key, value in metrics.items()}
        metrics_df = pd.concat(
            [metrics_df, pd.DataFrame(metrics, index=["mean"])]
        )

        # save the metrics
        metrics_df.to_csv(
            os.path.join(self.output_dir, "gene_embeddings_metrics.csv")
        )
    ...
    def evaluate(
        ...
        save_rankings: Optional[bool] = False,
    ) -> None:
        """
        ...
        if self.model_type == "scgpt":
            return self._evaluate_scGPT(
                remove_cell_embedding=remove_cell_embedding,
                include_zero_genes=include_zero_genes,
            )

        elif self.model_type == "geneformer":
            return self._evaluate_Geneformer(
                n_cells=n_cells,
                return_all=return_all,
                save_rankings=save_rankings,
            )

    def _visualize_Geneformer(
        self, n_cells: Optional[int] = 1000, cmap="Blues"
    ) -> plt.figure:
        ...
        if n_cells < n_all_available_cells:
            ...
            # select random cells
            rand_cells = np.random.choice(
                self.rankings_df["sample_id"].unique(), n_cells, replace=False
            )
            rankings_df = self.rankings_df[
                self.rankings_df["sample_id"].isin(rand_cells)
            ]
        elif n_cells < 1:
        ...
        fig, (ax1, ax2) = plt.subplots(
            ncols=2, sharey=True, tight_layout=True, figsize=(10, 5)
        )
        ...
        plt.suptitle(
            "Correlation between input and reconstructed and mean rankings"
        )

        save_path = os.path.join(self.output_dir, "input_and_outputs_kde.png")

        plt.savefig(save_path, bbox_inches="tight")
        if self._wandb:
            self._wandb.log(
                {"input_and_outputs_kde": self._wandb.Image(save_path)}
            )
        return plt

    def _visualize_scGPT(
        self, label_key: str = "cell_type", skip_cell: bool = True
    ) -> plt.figure:
        ...
        rand_indices = np.random.choice(
            np.arange(input_values.shape[0]), size=100, replace=False
        )
        ...
        with plt.style.context("ggplot"):
            ...
            ax, legend_axes = pych.composite(
                cmlist=[cm_values, cm_mlm_output, cm_mvc_output],
                main=0,
                legend_hpad=10,
                legend_gap=17,
            )
            ...
            ax.set_title(
                (
                    f"Comparison of input and output of the {model_run} "
                    "scGPT model"
                ),
                y=1.05,
            ...
            save_path = os.path.join(
                self.output_dir, "input_and_outputs_heatmap.png"
            )
            plt.tight_layout()
            plt.savefig(save_path, bbox_inches="tight")
            if self._wandb:
                self._wandb.log(
                    {"input_and_outputs_heatmap": self._wandb.Image(save_path)}
                )
        return plt
    ...
    def visualize(
        ...
        skip_cell: Optional[bool] = True,
        **kwargs,
    ) -> None:
        if self.model_type == "scgpt":
            self._visualize_scGPT(label_key=label_key, skip_cell=skip_cell)
    ...
    def plot_heatmap(
        ...
        skip_cell: bool = True,
    ) -> Optional[pych.ClusterMapPlotter]:
        ...
        mean_value_ = pd.Series(
            mean_value_, index=np.arange(mean_value_.shape[0])
        )
        rand_indices = list(range(mat.shape[0]))
        if mat.shape[0] > 100:
            msg = (
                f"Matrix is too large to plot ({mat.shape}), "
        ...
        if annot:
            ...
            cell_type_colors = dict(
                zip(
                    cell_types,
                    plt.cm.viridis(np.linspace(0, 1, len(cell_types))),
                )
            )
            # if cell annotation is not DataFrame or Series make it into one
            if (
                type(cell_type_annotation) != pd.DataFrame
                and type(cell_type_annotation) != pd.Series
            ):
                msg = (
                    f"cell_type_annotation is {type(cell_type_annotation)} "
                    "but must be a pandas DataFrame or Series; changing"
                )
                log.warning(msg)
                cell_type_annotation = pd.Series(
                    cell_type_annotation, index=rand_indices
                )
            row_ha = pych.HeatmapAnnotation(
                Cell=pych.anno_simple(
                    cell_type_annotation, colors=cell_type_colors, legend=True
                ),
                legend_gap=5,
                hgap=0.5,
                label_side="bottom",
        ...
        plot_df = pd.DataFrame(
            mat, index=range(mat.shape[0]), columns=range(mat.shape[1])
        )

        col_ha = pych.HeatmapAnnotation(
            Bin=pych.anno_barplot(mean_value_, legend=False),
            axis=1,
            plot_legend=False,
        )
