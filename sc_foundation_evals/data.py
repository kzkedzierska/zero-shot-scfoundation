...
class InputData:
    def __init__(self, adata_dataset_path: str) -> None:
    ...
    def add_batch_labels(
        ...
        if self.batch_key is None:
            ...
            batch_cols = [
                col for col in self.adata.obs.columns if "batch" in col.lower()
            ]
            if len(batch_cols) == 1:
                ...
                msg = (
                    "Cannot determine which column contains batch information!"
                )
                log.error(msg)
                raise ValueError(msg)
        else:
        ...
        self.adata.obs[self.batch_str_col] = self.adata.obs[
            ori_batch_col
        ].astype(str)
        batch_id_labels = (
    ...
    def preprocess_data(
        ...
        **kwargs,
        ...
        if gene_col not in self.adata.var.columns:
            ...
            log.warning(
                f"Gene names not found in var columns. Using index instead."
            )
        ...
        if model_type == "scgpt":
            ...
            self._preprocess_data_geneformer(
                preprocessed_path=preprocessed_path,
                save_ext=save_ext,
    ...
    def _preprocess_data_scGPT(
        ...
        # for filtering genes and cells, we need X to not be None
        if self.adata.X is None:
            if input_key not in self.adata.layers.keys():
                msg = (
                    f"Input key {input_key} not found in adata.layers! "
                    f"Available keys: {self.adata.layers.keys()}"
                )
                log.error(msg)
                raise ValueError(msg)
            self.adata.X = None
            self.adata.X = (
                self.adata.layers[input_key].toarray()
                if issparse(self.adata.layers[input_key])
                else self.adata.layers[input_key]
            )
        elif self.adata.X is not None and input_key != "X":
            msg = (
                "adata.X is not None, but input_key is not 'X'. "
                f"Overwriting X with {input_key}."
            )
            log.warning(msg)
            self.adata.X = (
                self.adata.layers[input_key].toarray()
        ...
        sc.pp.filter_genes(self.adata, min_counts=filter_gene_by_counts)
        sc.pp.filter_cells(self.adata, min_counts=filter_cell_by_counts)
        sc.pp.normalize_total(self.adata, target_sum=normalize_total)
        ...
        if preprocessed_path is not None:
            ...
            if os.path.exists(preprocessed_path):
                msg = (
                    f"Saving {self.dataset_name} preprocessed data "
                    f"to {preprocessed_path}"
                )
                self.adata.write(
                    os.path.join(preprocessed_path, f"{self.dataset_name}.h5ad")
                )
            else:
                msg = (
                    f"Directory {preprocessed_path} does not exist! "
                    "Skipping saving preprocessed data."
                )
                log.warning(msg)

    def _preprocess_data_geneformer(
        self,
        ...
        if preprocessed_path is None or not os.path.exists(preprocessed_path):
            msg = (
                "For Geneformer, preprocessed_path needs to be specified "
        ...
        if input_key is not None:
            ...
            if input_key not in self.adata.layers.keys():
                ...
                log.error(msg)
                raise ValueError(msg)
            self.adata.X = self.adata.layers[input_key]

        if input_key is None and self.adata.X is None:
            msg = (
                "No input key provided and no data found in adata.X! "
                "Please provide a valid input key."
            )
            log.error(msg)
            raise ValueError(msg)
        ...
        self.adata.var["ensembl_id"] = self.adata.var[self.gene_col].map(
            gene_name_id_dict
        )
        self.adata.var["has_ensembl_match"] = self.adata.var[
            "ensembl_id"
        ].notnull()
        ...
        self.adata.var.to_csv(
            os.path.join(preprocessed_path, f"{self.dataset_name}_var.csv"),
            index=False,
        )
        ...
        if save_ext == "loom":
            self.adata.write_loom(
                os.path.join(preprocessed_path, f"{self.dataset_name}.loom")
            )
        elif save_ext == "h5ad":
