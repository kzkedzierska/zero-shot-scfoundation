...
class CellEmbeddingsEval:
    def __init__(
        ...
        if not isinstance(
            ...
            msg = (
                "scgpt_instance must be an instance of "
        ...
        for label in self.label_key:
            if label not in self.data.adata.obs.columns:
                ...
                log.error(msg)
                raise ValueError(msg)
            self.data.adata.obs[label] = self.data.adata.obs[label].astype(
                "category"
            )
    ...
    def evaluate(
        ...
        label_cols = [
            x
            for i, x in enumerate(self.label_key)
        ...
        for label in label_cols:
            ...
            metrics = utils.eval_scib_metrics(
                adata_,
            ...
            for metric in metrics.keys():
                ...
                if self._wandb:
                    self._wandb.log(
                        {f"{embedding_key}/{label}/{metric}": metrics[metric]}
                    )
        ...
        met_df.to_csv(
            os.path.join(self.output_dir, f"{embedding_key}__metrics.csv"),
            index=False,
        )
    ...
    def create_original_umap(self, out_emb: str = "X_umap_input") -> None:
    ...
    def visualize(
        ...
        if "X_umap" in self.data.adata.obsm.keys():
            ...
            log.warning(
                f"Copying existing UMAP embedding to {old_umap_name} "
                "and overwriting X_umap."
            )
            self.data.adata.obsm[old_umap_name] = self.data.adata.obsm[
                "X_umap"
            ].copy()
        ...
        if adata_.n_obs > n_cells:
            ...
            adata_.obs.to_csv(
                os.path.join(self.output_dir, "adata_obs_subset.csv")
            )
        ...
        label_cols = [
            x for i, x in enumerate(label_cols) if x not in label_cols[:i]
        ]
        # remove label columns that are not in adata_.obs
        label_cols = [x for x in label_cols if x in self.data.adata.obs.columns]
        ...
        for i, label in enumerate(label_cols):
            ...
            if len(labels[label]) > 10:
                log.warning(
                    f"More than 10 labels for {label}."
                    f"The plots might be hard to read."
                )
            labels_colors[label] = dict(
                zip(
                    labels[label],
                    umap.generate_pallette(
                        n=len(labels[label]), cmap=palettes[i]
                    ),
                )
            )
        ...
        plot_type = [
            x for i, x in enumerate(plot_type) if x not in plot_type[:i]
        ]
        ...
        if len(plot_type) < len(old_plot_type):
            log.warning(
                f"Some plot type(s) {old_plot_type} is not valid. "
        ...
        if "simple" in plot_type:
            ...
            for i, label in enumerate(label_cols):
                ...
                axs[i].scatter(
                    ...
                    s=0.5,
                    c=[
                        labels_colors[label][x]
                        for x in self.data.adata.obs[label]
                    ],
                )
                legend_handles = [
                    axs[i].plot([], [], marker="o", ls="", color=c, label=l)[0]
            ...
            fig_savefig = os.path.join(
                self.output_dir, f"umap__{embedding_key}.png"
            )
            ...
            if self._wandb:
                self._wandb.log(
                    {f"umap__{embedding_key}": self._wandb.Image(fig_savefig)}
                )
            ...
            for i, label in enumerate(label_cols):
                if self.data.adata.obs[label].unique().shape[0] <= 10:
                    ...
                    if self._wandb:
                        self._wandb.log(
                            {
                                f"umap_wide__{embedding_key}_{label}": self._wandb.Image(
                                    wide_plot_savefig
                                )
                            }
                        )
                    ...
                    msg = (
                        f"More than 10 labels for {label}. Skipping wide plot."
                    )
                    log.warning(msg)
        ...
        if "scanpy" in plot_type:
            # scanpy plotting
            labels_colors_flat = {
                k: v for d in labels_colors for k, v in labels_colors[d].items()
            }
            if embedding_key == raw_emb:
                # TODO: this needs rewriting
                adata_temp__ = self.data.adata.copy()
                adata_temp__.obsm["X_umap"] = self.data.adata.obsm[
                    raw_emb
                ].copy()
                fig2 = sc.pl.umap(
                    adata_temp__,
            ...
            fig2_savefig = os.path.join(
                self.output_dir, f"umap_scanpy__{embedding_key}.png"
            )
            ...
            if self._wandb:
                self._wandb.log(
                    {
                        f"umap_scanpy/{embedding_key}": self._wandb.Image(
                            fig2_savefig
                        )
                    }
                )
